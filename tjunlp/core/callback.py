...
class Callback(object):
    ...
    @property
    def n_steps(self):
        """Trainer一共会采多少个batch。当Trainer中update_every设置为非1的值时，
        该值不等于update的次数"""
    ...
    def pbar(self):
        """
        如果在Callback中需要打印内容，请使用self.pbar.write(str)。否则可能出现命令行
        显示效果不太好的问题。在on_train_begin(), on_train_end(), on_exception()中
        请不要使用该属性，通过print输出即可。
        """

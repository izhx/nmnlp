...
from tjunlp.common.checks import ConfigurationError, check_for_gpu
...
from tjunlp.core.instance import Instance
from tjunlp.data.dataset import DataSet
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        if check_code_level > -1:
            # _check_code 是 fastNLP 帮助你检查代码是否正确的方法 。如果你在错误栈中
            # 看到这行注释，请认真检查你的field名与模型的输入名是否匹配
    ...
    def _train(self):
        ...
        self.epoch = 0
        start = time.time()
        with tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True) as pbar:
            self.pbar = pbar
            avg_loss = 0
